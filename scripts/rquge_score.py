...
class RQUGE(object):
    ...
    def predict_sp_score(self, input_sp):
        ...
        outputs = self.sp_scorer(input_ids=inputs["input_ids"].to(self.sp_scorer.device), \
                                 attention_mask=inputs["attention_mask"].to(self.sp_scorer.device))
    ...
    def scorer(self, context, pred_question, gold_answer, max_new_tokens=30):
        ...
        input_ids = self.tokenizer_qa.encode(input_string, return_tensors="pt").to(self.model_qa.device)
